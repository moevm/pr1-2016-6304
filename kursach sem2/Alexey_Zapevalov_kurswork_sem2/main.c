...
void findThem(char* number, char** arrayOneNoCommas, char** arrayTwoNoCommas)
...
void findGroups(char** arrayOne, char** arrayTwo) //разделяем данные, находим номера групп
{
    int i=0;
    ...
    while (groupNumbersArray[i][0]!=0)
    {
        ...
        findThem(groupNumbersArray[i], arrOneNoCommas, arrTwoNoCommas);
        i++;
    }
    i=0;
...
int main() //вызываются функции по созданию массивов, считыванию файлов и нахождению групп
{
